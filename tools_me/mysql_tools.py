...
class SqlData(object):
    ...
    def search_trans_money(self, start_t, end_t, user_sql=''):
        sql = "SELECT SUM(do_money) FROM user_trans WHERE do_type='充值' AND do_date BETWEEN  '{}' AND '{}' {}".format(start_t, end_t, user_sql)
        conn, cursor = self.connect()
        cursor.execute(sql)
        row = cursor.fetchone()
        self.close_connect(conn, cursor)
    ...
    def insert_card(self, card_number, cvc, expiry, card_id, last4, valid_start_on, valid_end_on, label, status, balance, user_id):
        sql = "INSERT INTO card_info(card_number, cvc, expiry, card_id, last4, valid_start_on, valid_end_on, label, " \
              "status, balance, user_id) VALUES('{}','{}','{}',{},'{}','{}','{}','{}','{}',{},{})"\
               .format(card_number, cvc, expiry, card_id, last4, valid_start_on, valid_end_on, label, status, balance, user_id)
        conn, cursor = self.connect()
        try:
            cursor.execute(sql)
            conn.commit()
    ...
    def search_account_trans(self, account_id, card_sql, time_sql, type_sql="", do_sql=""):
        sql = "SELECT * FROM user_trans WHERE user_id = {} {} {} {} {}".format(account_id, card_sql, time_sql, type_sql, do_sql)
        conn, cursor = self.connect()
        cursor.execute(sql)
        rows = cursor.fetchall()
        self.close_connect(conn, cursor)
    ...
    def search_class_action(self, class_info, subject, date_sql):
        sql = "SELECT pic_key FROM class_action WHERE class='{}' AND subject='{}' AND up_time='{}'".format(class_info, subject, date_sql)
        conn, cursor = self.connect()
        cursor.execute(sql)
        rows = cursor.fetchall()
        self.close_connect(conn, cursor)
        if not rows:
            ...
            url_list = list()
            for row in rows:
                url_list.append("http://cdn.trybest.top/" + row[0])
            return ','.join(url_list)

    def insert_class_action(self, up_time, class_info, subject, pic_key):
        sql = "INSERT INTO class_action(up_time, `class`, subject, pic_key) VALUES ('{}','{}','{}','{}')".format(
               up_time, class_info, subject, pic_key)
        conn, cursor = self.connect()
        try:
            cursor.execute(sql)
            conn.commit()
    ...
    def search_account_info(self):
        ...
        if not rows:
            ...
            for i in rows:
                ...
                account_dict['class'] = i[1]
                account_dict['level'] = i[2]
    ...
    def search_top_history(self, sql_line):
        sql = "SELECT * FROM top_up LEFT JOIN user_info ON user_info.id=top_up.user_id WHERE user_id != '' {}".format(sql_line)
        conn, cursor = self.connect()
        cursor.execute(sql)
        rows = cursor.fetchall()
        self.close_connect(conn, cursor)
    ...
    def search_trans_admin(self, cus_sql, card_sql, time_sql, do_sql, trans_type):
        sql = "SELECT user_trans.*, user_info.name FROM user_trans LEFT JOIN user_info ON user_trans.user_id" \
              " = user_info.id WHERE user_trans.do_date != ''  {} {} {} {} AND user_trans.trans_type='{}'".format(cus_sql, card_sql, time_sql,
                                                                                   do_sql, trans_type)
        conn, cursor = self.connect()
        cursor.execute(sql)
        rows = cursor.fetchall()

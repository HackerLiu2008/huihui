...
class SVB(object):
    ...
    def __init__(self):
        ...
        self.api_key = 'Bearer test_4uEqIutors4otTlFzMhhIrGQvWs6gUEg'
        # hmac_key, 构建请求头的关键参数之一
        self.hmac_key = 'qVfiUXtCxcabivlda744y6+WMGO22vv125JeEft872mMxEgUaust0p+N9XCHFsZk'
    ...
    def create_card(self, cents):
        ...
        try:
            ...
            resp = requests.post(url, headers=self.create_header(method, path, body=body), data=body, timeout=5)
    ...
    def update_card(self, card_id, cents):
        ...
        try:
            ...
            if status == 200:
                resp_data = resp.json()
                available_balance = resp_data.get('data').get('available_balance')
...
if __name__ == '__main__':
    ...
    res = svb.update_card(1883520, 10)
    print(res)
    print(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())))

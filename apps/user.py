...
def bento_refund():
    ...
    if res:
        ...
        return jsonify({"code": RET.OK, "msg": '退款成功！当前卡余额:$'+str(update_money/100)})
    else:
        return jsonify({"code": RET.SERVERERROR, 'msg': "网络繁忙,请稍后重试！"})
    ...
    if request.method == 'POST':
        ...
        if card_detail:
            ...
            if res:
                ...
                return jsonify({'code': RET.OK, 'msg': '充值成功！账户余额:$ ' + str(balance)+",卡余额:$ " + str(now_balance/100)})
            return jsonify({'code': RET.SERVERERROR, 'msg': '网络繁忙,请稍后重试！'})
        else:
            return jsonify({'code': RET.SERVERERROR, 'msg': '网络繁忙,请稍后重试！'})
...
def create_card():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return jsonify({"code": RET.OK, "msg": "成功开卡" + str(len(data_list)) + "张! 账户余额为: $"+str(balance)})
...
def push_log():
    ...
    try:
        ...
        if range_time or card_number:
            ...
            card_sql = ""
            if card_number:
                card_sql = " AND card_number LIKE '%" + card_number + "%'"

            sql = time_sql + card_sql
        else:
        ...
        if 'limit' in sql:
            ...
            for i in range(0, len(task_info), int(limit)):
                page_list.append(task_info[i:i + int(limit)])
            results['data'] = page_list[int(page) - 1]
            s = 'WHERE account_id={}'.format(user_id) + sql
        results['count'] = SqlData.search_value_count('push_log', s)
        return jsonify(results)
    except Exception as e:

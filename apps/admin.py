...
from tools_me.other_tools import admin_required, sum_code, xianzai_time, get_nday_list, now_day, save_file
...
def edit_parameter():
    if request.method == 'GET':
        class_info = request.args.get('class')
        subject = request.args.get('subject')
        t = request.args.get('date')
        context = dict()
        context['class_info'] = class_info
        context['subject'] = subject
        context['t'] = t
        return render_template('admin/edit_parameter.html', **context)
    if request.method == 'POST':
        results = {"code": RET.OK, "msg": MSG.OK}
        try:
            data = json.loads(request.form.get('data'))
...
def account_info():
    ...
    for i in task_one:
        ...
        if level == 'F':
            ...
            math_url = SqlData.search_class_action(class_info, 'math', now_t)
            lang_url = SqlData.search_class_action(class_info, 'lang', now_t)
            english_url = SqlData.search_class_action(class_info, 'english', now_t)
...
@admin_blueprint.route('/up_pic/', methods=['POST'])
def up_pic():
    if request.method == 'POST':
        file = request.files.get('file')
        class_info = request.args.get('class_info')
        subject = request.args.get('subject')
        t = request.args.get('t')
        file_name = t + "_" + class_info + '_' + subject + str(random.randint(1, 100)) +".png"
        path = save_file(file, file_name, 'H:\\huihui\\static\\huihui')
        status, url_key = up_photo(file_name, path, 'pay_pic')
        if status == 200:
            SqlData.insert_class_action(t, class_info, subject, url_key)
            return jsonify({'code': RET.OK, 'msg': MSG.OK})
        else:
            return jsonify({'code': RET.SERVERERROR, 'msg': MSG.SERVERERROR})
